...
`src-tauri/src/main.rs`); `bun run web` through the dev server's `/__settings`. So dev, an
installed build and a browser tab on one machine all share it. Nothing drawn before it is read:
...
- **`device`: this machine's own**: its id, the layout, chart panels and their indicators, the
  theme, the open list, sort and refresh interval.
